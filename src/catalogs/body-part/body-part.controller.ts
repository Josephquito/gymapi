import {
  Body,
  Controller,
  ForbiddenException,
  Get,
  Param,
  Patch,
  Post,
  Put,
  Req,
  UseGuards,
} from '@nestjs/common';
import { JwtGuard } from '../../auth/guards/jwt.guard';
import { CreateCatalogDto } from '../dto/create-catalog.dto';
import { UpdateCatalogDto } from '../dto/update-catalog.dto';
import { BodyPartService } from './body-part.service';

@Controller('body-parts')
@UseGuards(JwtGuard)
export class BodyPartController {
  constructor(private bodyPartService: BodyPartService) {}

  @Get()
  findAll() {
    return this.bodyPartService.findAll();
  }

  @Get('all')
  findAllAdmin(@Req() req: any) {
    if (req.user.role !== 'ADMIN') throw new ForbiddenException();
    return this.bodyPartService.findAllAdmin();
  }

  @Post()
  create(@Req() req: any, @Body() dto: CreateCatalogDto) {
    if (req.user.role !== 'ADMIN') throw new ForbiddenException();
    return this.bodyPartService.create(dto);
  }

  @Put(':id')
  update(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateCatalogDto) {
    if (req.user.role !== 'ADMIN') throw new ForbiddenException();
    return this.bodyPartService.update(id, dto);
  }

  @Patch(':id/deactivate')
  deactivate(@Req() req: any, @Param('id') id: string) {
    if (req.user.role !== 'ADMIN') throw new ForbiddenException();
    return this.bodyPartService.deactivate(id);
  }

  @Patch(':id/activate')
  activate(@Req() req: any, @Param('id') id: string) {
    if (req.user.role !== 'ADMIN') throw new ForbiddenException();
    return this.bodyPartService.activate(id);
  }
}
